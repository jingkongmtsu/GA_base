...
#else
#	setting=$src/setting
#	exe=$src/src/main/scf/SCF
#fi

exe="/opt/qchem/qc50/bin/qchem"
...
#if [[ -z $setting || -z $exe ]]
#then
#	echo "ERROR: source, executable, or setting is not set in $xTronOpt_config_file\n"
#	exit
#fi
...
cp $home_path/mol_sets/template_qchem.txt $job_path
...
#	fi
	...
	# EMUL_SETTINGS_FILE is where it looks for the xcfunc.conf to use which has the
	#  parameters set for this run
	# EMUL_BASIS_SET_HOME is where it will look for G3_LARGE.BAS or some other basis set file
	# EMUL_SCRATCH_DIR is for exporting the scratch directory to correct path
#	echo -e "\nexport EMUL_SETTING_FILE=$config_path
#export EMUL_BASIS_SET_HOME=$home_path/
#export EMUL_SCRATCH_DIR=$home_path/scratch\n" >> $config_path/iter$i.sh
#	echo -e "\nexport EMUL_SETTING_FILE=$config_path
#export EMUL_BASIS_SET_HOME=$home_path/
#export EMUL_SCRATCH_DIR=$home_path/scratch\n" >> $config_path/header.sh

	echo -e "\nexport QC=/opt/qchem/qc50
export QC_EXT_LIBS=/opt/qchem/QC_EXT_LIBS
export QCMAIN=/opt/qchem
export QCAUX=/opt/qchem/qcaux
export QCSCRATCH=/tmp/qcexample/scratchjk\n" >> $config_path/iter$i.sh
	echo -e "\nexport QC=/opt/qchem/qc50
export QC_EXT_LIBS=/opt/qchem/QC_EXT_LIBS
export QCMAIN=/opt/qchem
export QCAUX=/opt/qchem/qcaux
		...
		then
			...
			do
				...
				then
					...
					cat $base_files_path/$mol_set/$mol.in $base_files_path/template_qchem.txt > $job_path/$mol_set/${mol}_$i.in
					sed -i -r 's/%/$/' $job_path/$mol_set/${mol}_$i.in
